<template>
    <div v-if="_show">
        ...
        <div class="dialog-background" :style="{zIndex: this._zIndex -1}"></div>
    </div>
</template>
<script>
    /* ******************************************************************
        ...
        methods: {
            ...
            ******************************************************************* */
            postAndLoadSection: function () {
                ...
                this.$http.post('/api/' + this.page + '/' + action, this.viewModel)
                    .then(response => {
                        //Client processing
                        if (response.data._redirect && response.data._redirect.action) {
                            this.invoke(response.data._redirect)
                        } else {
