        ...
        methods: {
            ...
            ******************************************************************* */
            postAndLoadSection: function () {
                ...
                //パラメータなし
                var param = {};
                if (this.$route.query[this.section] === null) {
                    ...
                    //alert(issuer)
                    //var param = JSON.parse(decodeURIComponent(this.$route.query[this.section]))
                    param = this.$jwt.decode(this.$route.query[this.section], issuer);
                    if (param === null) param = {}
                ...
                this.$http.post('/api/' + this.page + '/' + action, copyObj)
                    .then(response => {
                        ...
                        if (response.data._redirect && response.data._redirect.action) {
                            ...
                            //    self.$emit('teller', self.section, { event: response.data._redirect }, response.data)
                            //}, 800, this);

                            this.$emit('teller', this.section, { event: response.data._redirect }, response.data)
                            //this.$emit('teller', this.section,response.data._redirect, response.data)
